...
# (ticks_left, ticks_right) and returns the new (x, y, heading angel).
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        # Get old center
        old_theta = old_pose[2]# cell to variavle
        old_x = old_pose[0]
        old_y = old_pose[1]#LiDAR

        # Modification: subtract(minus) offset which is the distance from scanner to center to compute center.
        old_x -= cos(old_theta) * scanner_displacement#vehicle center
        ...
        new_x = old_x + (R + robot_width/2.0) * (sin(new_theta) - sin(old_theta))#vehicle center
...
def compute_derivative(scan, min_dist):
    jumps = [ 0 ]
    for i in xrange(1, len(scan) - 1):  # from the second one to the last one
        l = scan[i-1]
        r = scan[i+1]
        if l > min_dist and r > min_dist:# valid measurement
            derivative = (r - l) / 2.0
...
def compute_cartesian_coordinates(cylinders, cylinder_offset):
    ...
    return result   # cartesian coordinates
...
def concatenate_transform(a, b):
    ...
    c = ca*cb - sa*sb
    s = sa*cb + ca*sb   # from the rotate matrix
