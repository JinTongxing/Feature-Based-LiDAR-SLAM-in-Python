...
class LegoLogfile(object):
    ...
    def read(self, filename):
        ...
        f = open(filename)
        for l in f: # For every row.
            sp = l.split()  # split each piece of information into tuple sp
            ...
            if sp[0] == 'P':
                if first_reference_positions:   # for the first time
                    self.reference_positions = []
                    first_reference_positions = False 
                self.reference_positions.append( (int(sp[2]), int(sp[3])) ) #store x,y only
            ...
            #   containing all scans, in scan_data.
                ...
                if s_record_has_count:
                    self.scan_data.append(tuple(map(int, sp[3:]))) # map function int all the members in tuple
                else:
            ...
            # Stored: List of (<type> info) tuples.
                ...
                if sp[1] == 'C':
                    ...
                    # Map function transport the string list into float list and the the plus merge two lists.
