...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius): # each frame
    cylinder_pairs = []
    ...
    for i in range(len(cylinders)):
        min_dist = max_radius # initial to the largest value
        closest_num = [] # initial to blank while another detected cylinder
...
if __name__ == '__main__':
    ...
    reference_cylinders = [l[1:3] for l in logfile.landmarks] # x, y, radius
    ...
    for i in xrange(len(logfile.scan_data)):
        ...
        world_cylinders = [LegoLogfile.scanner_to_world(pose, c)
                           for c in cartesian_cylinders] # x, y
