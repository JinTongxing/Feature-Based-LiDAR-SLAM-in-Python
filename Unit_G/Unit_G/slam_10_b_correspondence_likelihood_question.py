...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        #   which is np.dot(A, B).
        H = self.dh_dlandmark(self.pose, self.landmark_positions[landmark_number], scanner_displacement)  # Replace this.
        Sigma_k = self.landmark_covariances[landmark_number]
        Ql = np.dot(H, np.dot(Sigma_k, H.T)) + Qt_measurement_covariance  # Replace this.
        return (H, Ql)
