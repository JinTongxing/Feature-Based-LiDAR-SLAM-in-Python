...
class Distribution:
    ...
    def __repr__(self):
        ...
        for x in self.values:
            s += " %f" % x  # The link of several strings.
        return s
    ...
    def value(self, index):
        index -= self.offset
        # The new index is calculated from the first stored value.
    ...
    def plotlists(self, start = None, stop = None):
        if start == None:
            start = self.start()    # capture the distribution's initial offset as the start
        if stop == None:
            stop = self.stop()
        if start <= stop:
    ...
    @staticmethod   # back to do staticmethod first
