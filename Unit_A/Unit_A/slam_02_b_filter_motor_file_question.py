...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        Cx = old_pose[0] - cos ( old_pose[2] ) * ( scanner_displacement ) - ( R + robot_width / 2) * sin ( old_pose[2] )
        Cy = old_pose[1] - sin ( old_pose[2] ) * ( scanner_displacement ) - ( R + robot_width / 2) * ( -cos ( old_pose[2]) )
        theta = ( old_pose[2] + alpha ) % ( 2 * pi )
        x = Cx + ( R + robot_width / 2) * sin( theta ) + cos ( theta ) * ( scanner_displacement )
        y = Cy + ( R + robot_width / 2) * ( - cos( theta ) ) + sin ( theta ) * ( scanner_displacement )
        return (x, y, theta)
